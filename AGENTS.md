...
| 任务 | 相对阶段二 | 要求 |
|------|------------|------|
| 调制分类 | Δacc | **提升 ≥ 10 个百分点**（例：阶段二 80% → 阶段三 ≥ 90%） |
| 个体分类 | Δacc | **提升 ≥ 10 个百分点**（例：阶段二 70% → 阶段三 ≥ 80%） |

记录方式：在对应 `runs/experiments/<run>/` 保留 `train.log` / 监控指标与 `ckpts/best.ckpt`；对比时写明 stage2 / stage3 的 run 名与准确率数值。相对提升一律按**百分点**计算，不用相对比例。
