...
from resmamba_signal_model.models.heads import ClassificationHead
...
from resmamba_signal_model.models.task_interface import DEFAULT_TASKS, UniversalTaskInterface
...
def test_catalog_defaults_to_six_tasks() -> None:
...
def test_catalog_accepts_extra_pool_and_kind() -> None:
    catalog = resolve_task_catalog(
        {
            "task_pools": {
                "tx_modulation": ["a_train", "a_val"],
                "sonar": ["sonar_train", "sonar_val"],
            },
            "task_kinds": {"sonar": "classification"},
        }
    )
    assert "tx_modulation" in catalog.names
...
def test_catalog_tasks_list_can_be_arbitrary() -> None:
    ...
    assert catalog.names == ["tx_modulation", "nav_fix", "sonar"]
