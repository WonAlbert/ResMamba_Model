...
from resmamba_signal_model.data.packing import apply_segments, flip_segments
...
def _build_mamba2(d_model: int, *, d_state: int, d_conv: int, expand: int, headdim: int, ngroups: int = 1, chunk_size: int = 256) -> nn.Module:
    if _MAMBA2_AVAILABLE and _Mamba2Class is not None:
        return _Mamba2Class(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand, headdim=headdim, ngroups=ngroups, chunk_size=chunk_size)
    return _FallbackMamba2(d_model=d_model, d_conv=d_conv, expand=expand)
...
class BiMamba2Block(nn.Module):
    def __init__(self, d_model: int, *, d_state: int = 128, d_conv: int = 4, expand: int = 2, headdim: int = 64, dropout: float = 0.0, ngroups: int = 1, chunk_size: int = 256) -> None:
        super().__init__()
        self.norm = nn.LayerNorm(d_model)
        kwargs = dict(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand, headdim=headdim, ngroups=ngroups, chunk_size=chunk_size)
        ...
        *,
        ...
        if seq_idx is not None and cu_seqlens is not None:
            ...
            if x.is_cuda:
                ...
                bwd = flip_segments(self.bwd(h_rev, **mamba_kwargs), cu_seqlens)
            else:
...
class Mamba2Stack(nn.Module):
    def __init__(self, num_layers: int, *, d_model: int, d_state: int = 128, d_conv: int = 4, expand: int = 2, headdim: int = 64, dropout: float = 0.0, ngroups: int = 1, chunk_size: int = 256, activation_checkpointing: bool = False) -> None:
        super().__init__()
        self.activation_checkpointing = activation_checkpointing
        kwargs = dict(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand, headdim=headdim, dropout=dropout, ngroups=ngroups, chunk_size=chunk_size)
        self.layers = nn.ModuleList(BiMamba2Block(**kwargs) for _ in range(num_layers))
        self.norm = nn.LayerNorm(d_model)
